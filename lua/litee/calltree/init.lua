...
function M.open_to()
    ...
    if ctx.state == nil or ctx.state["calltree"] == nil then
        return
    end
...
function M.collapse_calltree()
    local ctx = ui_req_ctx()
    if ctx.state == nil or ctx.cursor == nil or ctx.state["calltree"].tree == nil then
        lib_notify.notify_popup_with_timeout("Must perform an call hierarchy LSP request first", 1750, "error")
        return
    end
    ctx.node.expanded = false
    lib_tree.remove_subtree(ctx.state["calltree"].tree, ctx.node, true)
    lib_tree.write_tree_no_guide_leaf(ctx.state["calltree"].buf, ctx.state["calltree"].tree, marshal_func)
...
function M.setup(user_config)
    local function pre_window_create(state)
        ...
        state["calltree"].buf = calltree_buf._setup_buffer(buf_name, state["calltree"].buf, state["calltree"].tab)
        if state["calltree"].tree == nil then
            return false
        end
        if state["calltree"].tree ~= nil then
            lib_tree.write_tree_no_guide_leaf(state["calltree"].buf, state["calltree"].tree, marshal_func)
        end
        return true
    end
    ...
    if not pcall(require, "litee.lib") then
        lib_notify.notify_popup_with_timeout(
            "Cannot start litee-calltree without the litee.lib library.",
            1750,
            "error"
        )
        return
